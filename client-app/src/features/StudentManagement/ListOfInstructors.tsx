...
import { Paper, Radio, RadioGroup, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow } from '@mui/material';
  ...
  return (
    <div>
        <Box sx={{  position: 'absolute' as 'absolute',top: '50%',left: '50%',transform: 'translate(-50%, -50%)',width: 1068,bgcolor: 'background.paper',boxShadow: 24,p: 4, background:'#f4f4f4',maxHeight:'495px',padding: '40px 80px 10px 80px'}}>
          ...
          <Button  sx={{position:'absolute', top:'0', right:'0', border:'none', marginTop:'10px',':hover': {border: 'none',background:'none'}}} variant="outlined" startIcon={<CloseIcon sx={{'&:nth-of-type(1)':{fontSize:'30px'}, color:'#333'}}/>} onClick={() => modalStore.closeModal()}></Button>
        ...
        <Button sx={{display:'flex', fontSize:'24px', textTransform:'capitalize',padding:'0px 40px', margin:'auto'}} variant="contained" onClick={() => modalStore.openModal(<UpdateSucces/>)}>Lưu</Button>
        </Box>
    </div>
  );
}