                ...
                <Box display="grid" gridTemplateColumns="repeat(12, 1fr)" gap={1}>
                    ...
                    <Box sx={{ height: 'fit-content', }} gridColumn="span 11">
                        ...
                        <Box {...getRootProps({ className: 'container_dropzone' })}>
                            {!(isDragActive || acceptedFiles.length > 0) && <UploadFileIcon className='icon_dropzone_' />}
                            <input {...getInputProps()} />
                        ...
                        {file && <Box className='dropzone_file_csv'>{}
                                <Button className={"icon_delete"} onClick={() => {
                                    ...
                                    sx={{ padding: '0px 0px 2px 50px', transform: 'translateX(8px)', position: 'absolute', bottom: '0', right: '0', border: 'none', ':hover': { border: 'none', background: 'none' } }} variant="outlined" startIcon={<DeleteForeverOutlinedIcon sx={{ '&:nth-of-type(1)': { fontSize: '15px' }, color: '#bebebe' }} />}
                                />
